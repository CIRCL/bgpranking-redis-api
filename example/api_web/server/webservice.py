...
def __default_dates_sources(req):
    ...
    if dates_sources is None:
        dates_sources = bgpranking.prepare_sources_by_dates(req.get('last_day'), req.get('timeframe'))
...
def __query_logging(ip, user_agent, method, date=None, source=None,
                    asn=None, asn_details=None, compared_asns=None,
                    ip_lookup=None, level=None):
    if level == 'warning':
